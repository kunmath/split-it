...
const activityItems = [
  ...
  "James opened the dinner expense draft.",
  "A placeholder invite route was generated for demo-token.",
];
  ...
  return (
    <PageContainer className="space-y-8">
      <div className="space-y-3">
        <p className="text-xs uppercase tracking-[0.24em] text-primary">Activity Placeholder</p>
        <h1 className="font-headline text-4xl font-extrabold tracking-tight text-on-surface sm:text-5xl">
          Quiet feed, correct shell
        </h1>
        <p className="max-w-2xl text-sm leading-8 text-on-surface-variant sm:text-base">
          This route proves the authenticated layouts, rail state, and utility bar without introducing real
          notification models yet.
        </p>
      </div>
      ...
      <div className="space-y-3">
        {activityItems.map((item) => (
          <SurfaceCard key={item} variant="low" className="flex items-start gap-4 rounded-[2rem] p-5">
            ...
            <div>
              <p className="font-headline text-lg font-semibold text-on-surface">Placeholder update</p>
